...
const ProductReview = ({ productId, averageRating, reviewCount, ratingDistribution }) => {
  ...
  const [reviewCounts, setReviewCount] = useState()

  const { data: reviewDetails } = useFetch(`/api/review/details?productId=${productId}`)

  useEffect(() => {
    if (reviewDetails && reviewDetails.success) {
      const reviewCountsData = reviewDetails.data
      setReviewCount(reviewCountsData)
    }
  }, [reviewDetails])
  ...
  const { error, data, isFetching, fetchNextPage, hasNextPage } = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => await fetchReview(pageParam),
    initialPageParam: 1,
    ...
    <div className="shadow rounded border mb-20">
      ...
      <div className="p-5">
        <div className="flex justify-between flex-wrap items-center">
          <div className="md:w-1/2 w-full md:flex md:gap-10 md:mb-0">
            <div className="md:w-[200px] w-full md:mb-0 mb-5">
              <h4 className="text-center text-8xl font-semibold">{reviewCounts?.averageRating}</h4>
              ...
              <p className="text-center mt-3">({reviewCounts?.totalReview} Rating & Reviews)</p>
            </div>
              ...
              <div className="w-full">
                {[5, 4, 3, 2, 1].map((rating) => (
                  <div key={rating} className="flex items-center gap-2 mb-2">
                    ...
                    <Progress value={reviewCounts?.percentage[rating]} className="h-2 w-50" />
                    <span className="text-sm">{reviewCounts?.rating[rating]}</span>
                  </div>
                ))}
              </div>
            </div>
