...
export async function POST(request) {
    try {
        ...
        const productSchema = z.object({
            ...
            mrp: z.union([z.number(), z.string().transform(Number)]),
            sellingPrice: z.union([z.number(), z.string().transform(Number)]),
        })
        ...
        const orderSchema = zSchema.pick({
            ...
            subTotal: z.union([z.number(), z.string().transform(Number)]),
            discount: z.union([z.number(), z.string().transform(Number)]),
            couponDiscountAmount: z.union([z.number(), z.string().transform(Number)]),
            products: z.array(productSchema)
        })

        const validate = orderSchema.safeParse(payload)
        if (!validate.success) {
            return response(false, 400, 'Invalid or Missing Filed.', { error: validate.error })
        }

        const validatedData = validate.data

        // payment verification
        const verfification = validatePaymentVerification({
