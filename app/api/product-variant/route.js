...
export async function GET(request) {
    try {
        ...
        if (globalFilter) {
            matchQuery['$or'] = [
                ...
                },
                {
                    $expr: {
                        $regexMatch: {
                            input: { $toString: { $ifNull: ["$discountPercentage", "$discountPercentages"] } },
                            regex: globalFilter,
                            options: 'i'
                        }
                    }
        ...
        delete countQuery['productData.name']
        const totalRowCount = await ProductVariantModel.countDocuments(countQuery)
